...
export default () => {
    ...
    const { currentPregnantLog, editSubmit } = usePregnantLogData();
        ...
        <div id="operation-item"
            ...
            <div id="header"
                ...
                <div id="header-title" className="flex gap-3 items-center h-9 py-1">
                    ...
                    <h1 className="font-sora text-lg sm:text-xl">Gebelik Takibi / Kayıt Düzenle / {currentPregnantLog._id}</h1>
                </div>
            </div>
                    ...
                    <form action=""
                        ...
                        <div className="grid grid-cols-2 gap-3 h-fit">
                            <select
                                ...
                                id="" defaultValue={currentPregnantLog.animal_id?._id} {...register("animal_id")}>
                                <option value="0" className="text-gray-400">Hayvanı seçiniz</option>
                                {animalArray?.map((animal: any) => (
                                    <option value={animal._id} className={`text-gray-400 ${(animal.gender === "male" || animal._id === currentPregnantLog.animal_id._id) && "hidden"}`}>{animal.name}</option>
                                ))}
                            </select>
                            ...
                            <input id="date-picker" type="date"
                                ...
                                className="col-span-2 border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary" defaultValue={moment(currentPregnantLog.pregnant_date).format("YYYY-MM-DD")} {...register("pregnant_date")}></input>
