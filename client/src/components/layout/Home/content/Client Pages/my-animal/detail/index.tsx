...
export default () => {
    const { currentAnimal } = useAnimalData();
    return (
        //  <!-- record detail - start -->
        <div id="operation-item"
            ...
            <div id="header"
                ...
                <div id="header-title" className="flex gap-3 items-center h-9 py-1">
                    ...
                    <h1 className="font-sora text-lg sm:text-xl">Hayvanlarım / Kayıt Detayı / {currentAnimal._id}</h1>
                </div>
            </div>
                    ...
                    <form action=""
                        ...
                        <div className="grid grid-cols-2 gap-3 h-fit">
                            <p
                                ...
                                Adı:{currentAnimal.name}</p>
                            <p
                                className="col-span-2 border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary">
                                Anne Adı:{currentAnimal.mother_id && currentAnimal.mother_id.name}</p>
                            <p
                                className="border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary">
                                Irkı:{currentAnimal.folk_id && currentAnimal.folk_id.name}</p>
                            <p
                                className="border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary">
                                Çip Durumu:{currentAnimal.chip_status}</p>
                            <p
                                className="border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary">
                                Doğum Tarihi:{new Date(currentAnimal.born_date).toLocaleDateString()}</p>
                            <p
                                className="border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary">
                                Cinsiyet:{currentAnimal.gender}</p>
                        </div>
                    </form>
                </div>
            </div>
