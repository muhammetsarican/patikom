...
export default (props: any) => {
    const { vaccineArray } = useVaccineData();
        ...
        <div id="operation-item"
            ...
            <div id="header"
                ...
                <div id="header-title" className="flex gap-3 items-center">
                    ...
                    <h1 className="font-sora text-lg sm:text-xl">Kayıtlı Aşılar / Kayıt Listesi</h1>
                </div>
            ...
            <div className="px-6">
                <div className="rounded-lg overflow-hidden border">
                    {!vaccineArray && <NoRecord />}
                    {vaccineArray && <table className="w-full text-xs sm:text-sm">
                        <thead className="bg-slate-50 text-accent-text">
                            <tr className="uppercase">
                                <th className="py-4">adı</th>
                                <th className="py-4">anahtar kelime</th>
                                <th className="py-4">işlemler</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white text-primary-text capitalize">
                            {vaccineArray.map((vaccine: any) => (
                                <ListItem props={props} vaccine={vaccine} />
                            ))}
                        </tbody>
                        {vaccineArray.length > 9 && <tfoot className="bg-slate-50 text-accent-text">
                            <tr className="uppercase">
