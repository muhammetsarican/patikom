...
export default () => {
    ...
    const { currentTreatment, editSubmit } = useTreatmentData();
        ...
        <div id="operation-item"
            ...
            <div id="header"
                ...
                <div id="header-title" className="flex gap-3 items-center h-9 py-1">
                    ...
                    <h1 className="font-sora text-lg sm:text-xl">Tedavi Geçmişi / Kayıt Düzenle / {currentTreatment._id}</h1>
                </div>
            </div>
                    ...
                    <form action=""
                        ...
                        <div className="grid grid-cols-2 gap-3 h-fit">
                            <input type="text" placeholder="Başlığı giriniz"
                                className="col-span-2 border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary" {...register("title")} defaultValue={currentTreatment.title}></input>
                            {errors.title && <ErrorText err={errors.title.message || ""} />}
                            <input type="text" placeholder="Anahtar kelimeleri giriniz"
                                className="col-span-2 border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary" {...register("keyword")} defaultValue={currentTreatment.keyword}></input>
                            {errors.keyword && <ErrorText err={errors.keyword.message || ""} />}
                            <input type="text" placeholder="Açıklama giriniz"
                                className="col-span-2 border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary" {...register("description")} defaultValue={currentTreatment.description}></input>
                            ...
                            <select
                                ...
                                id="" {...register("animal_id")} defaultValue={currentTreatment.animal_id?._id}>
                            ...
                            <select
                                className="border rounded-xl p-4 w-full bg-white text-xs text-gray-400 outline-none focus:outline-offset-0 focus:outline focus:outline-primary"
                                id="" {...register("vet_id")} defaultValue={currentTreatment.vet_id?._id}>
                            ...
                            <select
                                className="border rounded-xl p-4 w-full bg-white text-xs text-gray-400 outline-none focus:outline-offset-0 focus:outline focus:outline-primary"
                                id="" {...register("categories.0")} defaultValue={currentTreatment.categories[0]?._id}>
                            ...
                            <select
                                className="border rounded-xl p-4 w-full bg-white text-xs text-gray-400 outline-none focus:outline-offset-0 focus:outline focus:outline-primary"
                                id="" {...register("medicines.0")} defaultValue={currentTreatment.medicines[0]._id}>
