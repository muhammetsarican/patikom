...
export default (props: any) => {
    const { treatmentArray } = useTreatmentData();
        ...
        <div id="operation-item"
            ...
            <div id="header"
                ...
                <div id="header-title" className="flex gap-3 items-center">
                    ...
                    <h1 className="font-sora text-lg sm:text-xl">Tedavi Geçmişi / Kayıt Listesi</h1>
                </div>
            ...
            <div className="px-6">
                <div className="rounded-lg overflow-hidden border">
                    {!treatmentArray && <NoRecord />}
                    {treatmentArray && <table className="w-full text-xs sm:text-sm">
                        <thead className="bg-slate-50 text-accent-text">
                            <tr className="uppercase">
                                <th className="py-4">başlık</th>
                                <th className="py-4">hayvan adı</th>
                                <th className="py-4">veteriner</th>
                                <th className="py-4" colSpan={2}>işlemler</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white text-primary-text capitalize">
                            {treatmentArray.map((treatment: any) => (
                                <ListItem props={props} treatment={treatment} />
                            ))}
                        </tbody>
                        {treatmentArray.length > 9 && <tfoot className="bg-slate-50 text-accent-text">
                            <tr className="uppercase">
