...
export default () => {
    const { currentTreatment } = useTreatmentData();
        ...
        <div id="operation-item"
            ...
            <div id="header"
                ...
                <div id="header-title" className="flex gap-3 items-center h-9 py-1">
                    ...
                    <h1 className="font-sora text-lg sm:text-xl">Tedavi Geçmişi / Kayıt Detayı / {currentTreatment._id}</h1>
                </div>
            </div>
                    ...
                    <form action=""
                        ...
                        <div className="grid grid-cols-2 gap-3 h-fit">
                            <p
                                ...
                                Başlık: {currentTreatment.title}</p>
                            <p
                                className="col-span-2 border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary">
                                Anahtar Kelime: {currentTreatment.keyword}</p>
                            <p
                                className="col-span-2 border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary">
                                Açıklama: {currentTreatment.description}</p>
                            <p
                                className="border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary">
                                Hayvan Adı: {currentTreatment.animal_id?.name}</p>
                            <p
                                className="border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary">
                                Veteriner Adı: {currentTreatment.vet_id?.fname} {currentTreatment.vet_id?.lname}</p>
                            <p
                                className="border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary">
                                Kategori: {currentTreatment.categories[0]?.title}</p>
                            <p
                                className="border rounded-xl p-4 w-full text-xs outline-none focus:outline-offset-0 focus:outline focus:outline-primary">
                                İlaç: {currentTreatment.medicines[0]?.name}</p>
                        </div>
                    </form>
                </div>
            </div>
